...
def QS_070_Enable_Disable_POM(zq_run, zq_tu3,zq_enadis):

    zq_tl1_res=NE1.tl1.do("ED-TU3::{}::::POM={},EGPOM={};".format(zq_tu3,zq_enadis,zq_enadis))
    zq_msg=TL1message(NE1.tl1.get_last_outcome())
    dprint(NE1.tl1.get_last_outcome(),1)
    zq_cmd=zq_msg.get_cmd_status()

    if zq_cmd == (True,'COMPLD'):
        dprint("\nOK\tPom and EGPOM setting to [{}] for {} successful".format(zq_enadis,zq_tu3),2)
        zq_run.add_success(NE1, "Pom and EGPOM setting to [{}] for {} successful".format(zq_enadis,zq_tu3),"0.0", "Pom and EGPOM setting")

    else:
        dprint("\nKO\tPom and EGPOM setting to [{}] for {} failed".format(zq_enadis,zq_tu3),2)
        zq_run.add_failure(NE1,  "TL1 COMMAND","0.0", "Pom and EGPOM setting to [{}] for {} failed".format(zq_enadis,zq_tu3),"TL1 command fail "+ QS_000_Print_Line_Function())
...
class Test(TestCase):
    ...
    def test_setup(self):
        ...
        '''
        self.start_tps_block(NE1.id,"FM", "8-1-3-1")
    ...
    def test_body(self):
        ...
        ONT.get_set_rx_channel_mapping_size("P1", "VC3")
        ONT.get_set_rx_channel_mapping_size("P2", "VC3")
        
        ONT.get_set_tx_channel_mapping_size("P1", "VC3")
        ONT.get_set_tx_channel_mapping_size("P2", "VC3")
        ...
        self.stop_tps_block(NE1.id,"FM", "8-1-3-1")
