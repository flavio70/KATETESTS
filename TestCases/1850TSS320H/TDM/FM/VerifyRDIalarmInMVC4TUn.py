...
:field Description: This test verifies the detection/clearance of RDI alarm on MVC4TU3 and TU12 facilities
:field Topology: 5
:field Dependency:
:field Lab: SVT
...
def QS_070_Enable_Disable_POM(zq_run, zq_mtx_slot, zq_vc4, zq_enadis):
    ...
    for zq_j in range (1,4):
        ...
        if zq_cmd == (True,'COMPLD'):
            ...
            zq_run.add_failure(NE1,  "TL1 COMMAND","0.0", "Pom and EGPOM setting to [{}] for MVC4TU3-{}-{}-{} failed".format(zq_enadis,zq_mtx_slot, zq_vc4, zq_j),"TL1 command fail")
...
def QS_100_Check_RDI(zq_run, zq_ONT_p1, zq_ONT_p2, zq_mtx_slot, zq_vc4_1, zq_vc4_2):
    ...
    for zq_j in range (1,4):
        ...
        if (zq_msg.get_cmd_response_size() == 0):
            ...
            zq_run.add_failure(NE1,"RFI Condition verification failure for LOVC3 facility : Exp [6] - Rcv [0]","0.0", "RFI Condition verification failure: Exp [6] - Rcv [0]","RFI CONDITION CHECK")
        else:
            zq_cmd=zq_msg.get_cmd_status()
            if zq_cmd == (True,'COMPLD'):
                zq_rfi_nbr = 0
            ...
            if zq_rfi_nbr == 6:
                ...
                zq_run.add_failure(NE1,"RFI Condition verification failure for LOVC3 facility : Exp [6] - Rcv [{}]".format(zq_rfi_nbr),"0.0", "RFI Condition verification failure: Exp [1] - Rcv [{}]".format(zq_rfi_nbr),"RFI CONDITION CHECK")
        ...
        if (zq_msg.get_cmd_response_size() == 0):
            ...
            if zq_rfi_nbr == 6:
                ...
                dprint("KO\tRFI Condition verification failure for LOVC3 facility : Exp [6] - Rcv [{}]".format(zq_rfi_nbr),2)
                zq_run.add_failure(NE1,"RFI Condition verification failure for LOVC3 facility : Exp [6] - Rcv [{}]".format(zq_rfi_nbr),"0.0", "RFI Condition verification failure: Exp [6] - Rcv [{}]".format(zq_rfi_nbr),"RFI CONDITION CHECK")
